			...
			if(res.status == 0){
				...
				if(res.data.notifyList.length>0){
					for(var i=0;i<res.data.notifyList.length;i++){
						var partContent = '<div class="circleHidden"></div>';
						if(!res.data.notifyList[i].nuTime){
							partContent = '<div class="circleRed"></div>';
						}
						content += '<li><a href="/myBooksInfo/messagesDetail_admin?nid='+res.data.notifyList[i].nid+'">'+
							'<div class="readMark">'+partContent+'</div>'+
